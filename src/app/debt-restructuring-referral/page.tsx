...
export default function DebtRestructuringReferral() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const ok = await submit(formData, {
      source: "debt-restructuring-referral-page",
    });
    if (ok) {
      setFormData({
        fullName: "",
        ...
        <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-brand-blue text-white font-semibold py-2 px-4 rounded-md hover:bg-[#0e487d] disabled:bg-slate-400 transition-colors"
            >
              {status === "submitting"
                ? "Submitting..."
                : "Submit For Referral"}
